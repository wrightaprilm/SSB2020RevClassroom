...
![](images/journal.pone.0109210.g003.png)
...
library(ggplot2)
gamma_est <- read.csv("output/mk_run_1.log", sep = "\t")
log_est <- read.csv("output/mk_lognormal_run_2.log", sep = "\t")

density_gamma <- density(gamma_est$Posterior)
density_log <- density(log_est$Posterior)

p1 <- ggplot(data = log_est, aes(x=Posterior))+
  geom_density() +
  geom_density(data=gamma_est, fill="purple") +
...
![tutorials](images/RB_tutorial.png)
